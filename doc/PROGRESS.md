...
| §2 Security Hardening | 4 | 0 | 4 | 0 | 8 |
...
| §8 Functional Improvements | 0 | 0 | 1 | 0 | 1 |
| **Total** | **29** | **10** | **45** | **3** | **87** |
...
### [pending]
- **Self-signed certificate option in the request flow.** Add a
  "Self-signed" entry to the CA selector on the certificate-request
  page (and a matching API path). The selected template still drives
  subject DN / validity / extensions; the freshly-generated private
  key signs the certificate as its own issuer. The KMS path is
  unchanged — the cert key still goes through the default software
  provider. The result is recorded with `Certificates.ca_id = NULL`
  (already nullable) plus a new `is_self_signed` boolean so the
  certificate list can show the distinction without a JOIN. Likely
  touch points: web/templates/certificate_request.html, the request
  handler in web/routes/main_routes.py, the Certificates schema in
  pypki/db.py.
